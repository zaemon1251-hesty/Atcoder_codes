...
def convert_to_milliseconds(dt: list) -> int:
    dt = list(map(int, dt))
    milliseconds = (dt[0] * 3600 + dt[1] * 60 + dt[2]) * 1000 + dt[3]
    return milliseconds
...
def is_peak(time: int):
    time %= 24 * hour_base
    return 7 * hour_base <= time < 9 * hour_base + 30 * minute_base or 18 * hour_base <= time < 24 * hour_base
...
def process_file(input_file_path) -> deque:
    ...
    for i, record in enumerate(records):
        ...
        if (
            len(time_str) != 12
            or not time_str[:2].isdigit()
            or not time_str[3:5].isdigit()
            or not time_str[6:8].isdigit()
            or not time_str[9:].isdigit()
            or not distance_str.replace(".", "").isdigit()
        ):
            raise ValueError
...
def get_price_regard_distance_time(distance: int, distance_state: DistanceState, is_midnight: bool, is_peak: bool):
    ...
    if distance_state == DistanceState.free:
        ...
        _price += SHORT_DISTANCE_FARE_RATE * (distance // 4000)
        _distance_mod = distance % 4000
    else:
        _price += SHORT_DISTANCE_FARE_RATE * (distance // 3500)
...
def get_price_regard_initial_fare(distance: int, is_midnight: bool, is_peak: bool):
    assert distance == 0
    assert is_midnight is False or is_peak is False
...
def calc_price(que: deque):
    ...
    def process_first_ride(time, distance, is_midnight, is_peak):
        ...
        total_fare += get_price_regard_initial_fare(distance, is_midnight, is_peak)
    ...
    def process_meter(time, distance, is_midnight, is_peak):
        ...
        price, distance_mod = get_price_regard_distance_time(distance_diff, state, is_midnight, is_peak)
